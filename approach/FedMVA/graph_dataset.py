...
class DataSet:
    ...
    def get_valid_data(self):
        features = np.array([entry.features for entry in self.valid_entries])
        targets = np.array([entry.label for entry in self.valid_entries])
    ...
    def initialize_dataset(self, balance=True, output_buffer=sys.stderr):
        if isinstance(balance, bool) and balance:
            ...
            smote = SMOTE(random_state=1000, k_neighbors=2)
...
def create_dataset(train_file, valid_file=None, test_file=None, batch_size=128, output_buffer=sys.stderr):
    if output_buffer is not None:
    ...
    print(f"Feature dimension (hdim): {hdim}")  # 打印特征维度信息
