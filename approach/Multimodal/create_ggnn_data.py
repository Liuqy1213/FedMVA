...
keywords = ["alignas", "alignof", "and", "and_eq", "asm", "atomic_cancel", "atomic_commit",
            ...
            "def", "import", "class", "Spring", "Django"]
puncs = '~`!@#$%^&*()-+={[]}|\\;:\'\"<,>.?/'
puncs = list(puncs)
...
def inputGeneration(nodeCSV, edgeCSV, target, wv, edge_type_map, cfg_only=False):
    ...
    with open(nodeCSV, 'r') as nc:
        ...
        for idx, node in enumerate(nodes):
            if node is not None and node.get('isCFGNode') is not None:
            ...
            node_feature = inputGeneration2(node_type, type_map, type_one_hot)
...
def inputGeneration2(node_type, type_map, type_one_hot):
    # 检查 node_type 是否在 type_map 中
    if node_type in type_map:
        # 如果在 type_map 中，获取对应的特征
        node_feature = type_one_hot[type_map[node_type] - 1].tolist()
    else:
        # 如果不在 type_map 中（例如 'Directory'），则跳过或处理这种情况
        print(f"Warning: '{node_type}' not found in type_map. Skipping...")
        node_feature = []  # 或者设置为其他适当的默认值
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='bigvul')
    parser.add_argument('--csv', help='normalized csv files to process', default='D:\\VDBFL\\new_dataset\\csv_file')
    parser.add_argument('--src', help='source c files to process', default='D:\\VDBFL\\new_dataset')
    # parser.add_argument('--wv', default='../data/data/chrome_debian/raw_code_deb_chro.100')
    parser.add_argument('--wv', default='D:/VDBFL/data_processing/model/wv_models/datasets')
    parser.add_argument('--output', default='D:/VDBFL/data/ggnn_data/')
    args = parser.parse_args()
    json_file_path = 'D:/VDBFL/data/' + args.project + '_full_data_with_slices.json'
    ...
    for didx, entry in enumerate(tqdm(data)):
        file_name = entry['file_path'].split('new_dataset')[1]
        print(file_name)
        nodes_path = os.path.join('D:/VDBFL/csv_file', file_name, 'nodes.csv')
        edges_path = os.path.join('D:/VDBFL/csv_file', file_name, 'edges.csv')
    ...
    output_path = 'D:/VDBFL/data/ggnn_data/.shard' + str(data_shard)
