...
parser.add_argument('--base', help='paths to loaction of json shards to be ccombined', default='D:\\VDBFL\\data\\ggnn_data')
parser.add_argument('--model', help='paths to word2vector model', default='D:\\VDBFL\\data_processing\\model\\wv_models\\datasets')
parser.add_argument('--output', help='output directory for resulting json file', default='prep_script/')
...
print("Loaded word vector model",wv_model_li)

# ggnn_data = []

def extract_line_graph_data(project, base_dir=args.base, output_dir=args.output):
    if project == 'devign':
    ...
    for sc in range(1, shard_count):
        ...
        try:
            for data in tqdm(shard_data):
                ...
                nodes_file_path =  "D:\\VDBFL\\new_dataset\\csv_file\\" +file_name.strip()+'\\nodes.csv'
                edges_file_path =  "D:\\VDBFL\\new_dataset\\csv_file\\" +file_name.strip()+'\\edges.csv'
    ...
    print(os.path.join(output_dir, project + '-line-ggnn.json'))
    json.dump(graphs, output_file)
    output_file.close()
    print(len(graphs))
#     return graphs
