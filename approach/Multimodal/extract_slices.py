...
def read_csv(csv_file_path):
    data = []
    with open(csv_file_path,encoding='ISO-8859-1') as fp:
        header = fp.readline()
        header = header.strip()
    ...
    for node_index, node in enumerate(nodes):
        ...
        if 'location' in node.keys():
            ...
            try:
                ...
                print(f"Skipping invalid location value: {location}")
...
def create_forward_slice(adjacency_list, line_no):
    ...
    while len(stack) != 0:
        ...
        # print(cur)
        if cur not in adjacency_list.keys():
            continue
        else:
...
def tokenize(file_text):
    try:
        c_file = open('D:\VDBFL\data_processing\\tmp\\test1.c', 'w')

        c_file.write(file_text)
        c_file.close()
        tok = Tokenizer('D:\VDBFL\data_processing\\test1.c')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', help='name of project for differentiating files', default='bigvul')
    parser.add_argument('--input', help='directory where raw code and parsed are stored', default='D:\\VDBFL\\new_dataset')
    parser.add_argument('--text_in', help='path to cfg_full_text_files', default='D:\\VDBFL\\data\\ggnn_input\\datasets')
    # parser.add_argument('--output', help='output path for sliced full data json', default='../data1/')
    parser.add_argument('--output', help='output path for sliced full data json', default='../data/')
    args = parser.parse_args()

    split_dir = args.input
    print(split_dir)
    parsed = '../data/'+args.project+'/csv_file/'

    # ggnn_json_data = json.load(open(args.text_in + args.project + '_cpg_full_text_files.json'))
    # print("ggnn_json_data:", ggnn_json_data)
    # bigvul_cpg_full_text_files.json
    ggnn_json_data = json.load(open('D:\\VDBFL\\data\\ggnn_input\\bigvul_cpg_full_text_files.json'))
    files=[]
    for d in ggnn_json_data:
        if d is not None and d.get('file_name') is not None:
            print(d['file_name'])
    ...
    for i, file_name  in enumerate(files):
        ...
        code_text = read_file( 'D:\VDBFL\\raw_data\BigVul\\'+file_name.strip())

        print(file_name)
        nodes_file_path = 'D:\\VDBFL\\csv_file\\' + file_name.strip() + '\\nodes.csv'
        edges_file_path = 'D:\\VDBFL\\csv_file\\' + file_name.strip() + '\\edges.csv'
        with open(nodes_file_path, 'r', encoding='ISO-8859-1') as nc:
        ...
        for node_idx, node in enumerate(nodes):
            ...
            elif 'operator' in node and node['operator'] is not None and node['operator'].strip() in ['+', '-',
                                                                                                              '*', '/']:
                    line_no = extract_line_number(node_idx, nodes)
                    if line_no > 0:
    ...
    print(len(all_data))
    # output_file = open('D:/VDBFL/data1/' + args.project + '_full_data_with_slices.json', 'w')
    output_file = open('D:/VDBFL/data/' + args.project + '_full_data_with_slices.json', 'w')
